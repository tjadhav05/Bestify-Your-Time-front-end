...
import { BestScoreManager } from 'src/app/services/app.storage.service';
...
export class SnakeGameComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private bestScoreService: BestScoreManager, private footer: FooterService
  ) {
    this.setBoard();
  ...
  ngOnDestroy(): void {
    this.bestScoreService.destroyScore();
  }

  //on key press >^< snake direction change
  ...
  //for abstacles on board
  addObstacles(): void {
    let x = this.randomNumber();
    let y = this.randomNumber();

    //if box is not free then dont add obstacles
    if (this.board[y][x] === true || y === 8) {
      return this.addObstacles();
    }

    //if not free push it into array
    this.obstacles.push({
      x: x,
      y: y
    });
  ...
  //check obstacle present on that index or not
  ...
  //for generating random number for obstacles randomly
  randomNumber(): any {
